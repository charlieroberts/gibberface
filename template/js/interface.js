...
Interface.Widget = {
  ...
  setValue : function(value, doNotDraw) {
    ...
    if(this.target) {
      console.log("SENDING TARGET FOR", this.type)
  ...
  Interface.extend(this, {
    ...
    changeValue : function( touch, xOffset, yOffset ) {
      if(this.hasFocus || !this.requiresFocus) {
        ...
        var range = this.max - this.min;
        this.values[touch.id].x = this.min + range * (xOffset / this._width());
        this.values[touch.id].y = this.min + range * (yOffset / this._height());
        shouldrunvaluechange = true;
    ...
    trackMouse : function(xPos, yPos, id) {
      ...
      var touchFound = this.children[0];
      var touchNum = 0;
      for(var i = 0; i < this.children.length; i++) {
...
Interface.MultiSlider = function() {
  ...
  var half = this.max - this.min / 2;
  
  for(var i = 0; i < this.count; i++) {
    this._values[i] = half;
    this.values[i] = this.min + (this.max - this.min) * half;
  }
};
...
Interface.MultiButton = function() {
  Interface.extend(this, {
    ...
    draw : function() { 
      ...
      for(var i = 0; i < this.rows; i++) {
        for(var j = 0; j < this.columns; j++) {
          ...
          if(this.children[ btn ].label) {
            this.ctx.fillStyle = this._stroke();
            this.ctx.textBaseline = 'middle';
            this.ctx.textAlign = 'center';
            this.ctx.font = this._font();
            this.ctx.fillText(this.children[ btn ].label, _x + childWidth / 2, _y + childHeight / 2);
          }
        }
      }
    },
    ...
    defineChildren : function() {
      for(var i = 0; i < this.rows; i++) {
        for(var j = 0; j < this.columns; j++) {
          var btn = i * this.columns + j;
  ...
  Object.defineProperties(this, {
    ...
    rows : {
      ...
      set : function(_rows) { rows = _rows; this.defineChildren(); this.refresh(); },
    },
    columns : {
      get : function() { return columns; },
      set : function(_columns) { columns = _columns; this.defineChildren(); this.refresh(); },
    },
  });
};
